...
calculate_progress <- function(stage, weekly_hours) {
  ...
  for (i in seq_len(nrow(weekly_data))) {
    stage <- weekly_data$Stage[i]
    cum_hours <- fluency_stages$CumulativeHours[stage - 2]
    hours_spent <- weekly_data$HoursSpent[i]
    current_stage_cum_hours <- fluency_stages$CumulativeHours[stage - 2]
    next_stage = fluency_stages$CumulativeHours[stage - 1]
    cat(stage, cum_hours, hours_spent, current_stage_cum_hours, next_stage, "\n")
    p <- ((hours_spent - current_stage_cum_hours) / ( next_stage - current_stage_cum_hours)) + stage
    progress[i] <- p
...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(4, 
           ...
           h3(htmlOutput("savingsText"),style=" margin-top: 24px; line-height:2;"),
  ...
  fluidRow(
    ...
    column(8, 
           ...
           plotOutput("fluencyGraph", width = "80%")
           )
    ),
    ...
    column(4, 
          ...
          style = "margin-right: 50px;",
           plotOutput("costComparison", width = "100%")
           )
    )
  )
)
...
server <- function(input, output) {
  ...
  observeEvent( list(input$ability,input$hours), {
    ...
    integer_points <- progress_data %>%
      group_by(GraphLabel) %>%
      filter(HoursSpent == min(HoursSpent)) %>%
      ungroup()  
    ...
    output$costComparison <- renderPlot({
      ...
      # Example data for plotting
        ggplot(data, aes(x = Category, y = PlotCost, fill = Category)) +
        geom_col(show.legend = FALSE, width = 0.8, position = position_dodge(width = 0.9)) +  # Adjust width for spacing
        geom_text(aes(label = scales::dollar(Cost)), vjust = 3, family = "Helvetica", size = 8, color = "white", face = "bold", position = position_dodge(width = 0.5)) +  # Adjusted vjust and position dodge
        ...
        theme(
          ...
          #panel.border = element_rect(color = "black", fill = NA, size = 1),  # Add border to the panel area
          #plot.background = element_rect(color = "grey", size = 1)  # Add border around the entire plot area
        ) +
    ...
    HTML(paste0("Compared to a pay-per-hour tutor, you would <span style='color: #D17329; font-weight: bold; line-height: 2; '>save around $", round(tutor_cost - cost), "</span> by practising with Hey Lady!"))
    })
